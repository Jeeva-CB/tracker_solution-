...
function TeamMembers() {
    ...
    //var draftid = $("[id*=hddraftid]").val();
    var CRFID = $("#ddlCrf").val();
    var rqstid = $("[id*=hdRqstID]").val();
    //var draftid = $("[id*=hddraftid]").val(CRFID);
    //alert(CRFID);
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "TechnicalAnalysisTesting.aspx/fillTable",
        data: "{pageVal:'TestingTeamMember', pageval1 :'" + usr + "', pageval2: '" + rqstid + "'}",
        dataType: "json",
        success: function (Result) {
            ...
            //$('#ddlTester').empty();
            //if (Result != null) { 
            //    $('#ddlTester').append($("<option></option>").val("-1").html("Choose Test Engineer"));
            //    valData = Result.split('§');
            //    for (i = 0; i < valData.length - 1; i++) {
            //        valData1 = valData[i].split('^');
                   
            //        $('#ddlTester').append($("<option></option>").val(valData1[0] + '^' + valData1[3]).html(valData1[1]));
            //    }
...
function fillTableDtl(data) {
    ...
    if ($("#tblEmps tr").length == 0) {
        $("#tblEmps").empty();
        $('#tblEmps').append('<tr style="background-color:honeydew;color:black"><th class="text-center">EmpCode</th><th class="text-center">EmpName</th><th class="text-center">Work Assigned Till</th></tr>');
    }
    for (i = 0; i < valData.length - 1; i++) {
        valData1 = valData[i].split('^');
        $('#tblEmps').append('<tbody><tr class="text-center" style="background-color:white" >' +
            '<td>' + valData1[0] + '</td>' +
            '<td>' + valData1[1] + '</td>' +
...
function StartDateFill() {
    ...
    $("#txtStartDt").val($('#ddlTester').val().split('^')[1]);
}
...
function CalTotal() {   
    ...
    var data1,totm = 0, totc = 0, cst=600, wrkhr, noc, data = "", totman = 0, totcst = 0, tstval, tstrslt, totdev=0, tottst=0,totcod=0,totvapt=0;
    ...
    if (flag == 1) {
        ...
        $.ajax({
            ...
            success: function (Result) {
                ...
                data = $('#ddlProjType option:selected').text() + "^" + $('#ddlPhase option:selected').text() + "^" + $('#ddlRelWork option:selected').text() + " X " + tstval + "^" + $("#txtStartDt").val() + "^" + $("#txtEndDt").val() + "^" + wrkhr + "^" + tstval + "^" + testrow + "^" + $('#ddlTester option:selected').text() + "^" + $("#ddlTester").val().split('^')[0] + '^' + module + "¶";
...
function filltab(data) {
    ...
    valData = data.split('¶');
     ...
     if ($("#tabChange tr").length == 0) {
          $("#tabChange").empty();
         $('#tabChange').append('<tr style="background-color:honeydew;color:black"><th class="text-center">Project Type</th><th class="text-center">Phase</th><th class="text-center">Related Work</th><th class="text-center">StartDate</th><th class="text-center">EndDate</th><th class="text-center">TestCases</th><th class="text-center">Test Engineer</th><th class="text-center">DELETE</th></tr>');
     }
      for (i = 0; i < valData.length - 1; i++) {
          valData1 = valData[i].split('^');
          if (valData1[7] != "") {
        ...
        $('#tabChange').append('<tbody><tr>' +
            ...
            '<td style="display:none">' + valData1[5] + '</td>' +
            '<td style="text-align:center">' + valData1[6] + '</td>' +
            '<td style="display:none">' + valData1[7] + '</td>' +
            '<td style="text-align:center">' + valData1[8] + '</td>' +
            '<td style="display:none">' + valData1[9] + '</td>' +



           //<td style="text-align:center">' + valData1[10] + '</td>' +
            //'<td style="text-align:center">' + valData1[1] + '</td>' +
            //'<td style="text-align:center">' + valData1[2] + '</td>' +
            //'<td style="text-align:center">' + valData1[3] + '</td>' +
            //'<td style="text-align:center">' + valData1[4] + '</td>' +
...
function AddDetails() {
    if ($('#ddlTester').val() == "-1") {
        Swal.fire("Please Select Tester");
        return false;
    }
    $('#boxTestEng').hide();
    $('#boxdata').show();
...
function calTotdtl(data) {  
    ...
    var valData, valData1, cst=600, totman = 0, totcst = 0, tstval, tstrslt, totdev = 0, tottst = 0, totcod = 0, totvapt = 0;   
    valData = data.split('¶');
    for (i = 0; i < valData.length - 1; i++) {
        valData1 = valData[i].split('^');
        tottst = tottst + parseInt(valData1[7]);
        workHours2 = parseInt(workHours2) - parseInt(valData1[7]);
...
function businessHRFill() {
    ...
    if (checkHours == 1) {
        ...
        $("#TCost").val(parseFloat(parseInt($("#TWork").val()) * parseInt('600')).toFixed(2));
        ...
        var dateDifference = NewToDate - newFrmDate;
        if (dateDifference < 0) {
            Swal.fire("Choose To Date greater than Start Date");
            $("#txtEndDt").val("");
            $("#txtEndDt").focus();
            return false;
        }
...
//function Add_btn_Clear() {
//    //$('#ddlTester').val("-1");
//    //$('#ddlTester').html("Choose Test Engineer");
//    //$('#ddlPhase').val("-1");
//    //$('#ddlPhase').html("Choose Phase");
//    //$('#ddlRelWork').val("-1");
//    //$('#ddlRelWork').html("Choose Related Work");
//    //$('#ddlProjType').val("-1");
//    //$('#ddlProjType').html("Choose Project Type");
//    $('#TNoc').val("");
//    document.getElementById("ddlTester").value = "-1"; 
//    document.getElementById("ddlPhase").value = "-1"; 
//    document.getElementById("ddlRelWork").value = "-1"; 
//    document.getElementById("ddlProjType").value = "-1"; 
//    $('#txtStartDt').val("");
//    $('#txtEndDt').val("");
//   // $('#lblBusHours').html("");
    
//}
function ApplicableHideT() {
    if ($('#ddlCrf').val() == -1) {
        alert("Please Select a CRF..");
...
function techlead() {
    ...
    $.ajax({
        ...
        data: "{pageVal:'TestTechLead', pageval1 :'" + usr + "'}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
            // alert(Result);
...
function HandOverTest() {
    ...
    if (proceed) {
        ...
        $.ajax({
            ...
            data: "{pageVal:'HandOver', pageval1 :'" + Tech + "',pageval2:'" + $("[id*=hdRqstID]").val() + "',pageval3:'2'}",
            dataType: "json",
            async: false,
            success: function (Result) {
                $('.block-ui').addClass('clear');
...
function TestingTAConfirm(circularStatus) {
    ...
    var Data = '', itmdata = '', flag = 0, module = "";
    var table = document.getElementById('tabChange');
    var rowLength = table.rows.length;
    var reqid = $("[id*=hdRqstID]").val();
    var dtls, totman = 0, cst = 600, totcst = 0;
    var rmk = $('#txtRemarks').val();
    rbmodl = $("input:radio[name=rbmodl]:checked").val();
    if (rbmodl == '0') module = "Low";
    else if (rbmodl == '1') module = "Medium";
    ...
    //} 
    if (circularStatus == 0) {
        Data = $("[id*=hddraftid]").val() + 'µ' + reqid + 'µ' + $("[id*=hdUserId]").val() + 'µ' + $('#DevWork').val() + 'µ' + $('#TstWrk').val() + 'µ' + $('#CodRvw').val() + 'µ' + $('#TWork').val() + 'µ' + $('#TCost').val() + 'µ' + rmk + 'µ' + circularStatus + 'µ' + module + 'µ';
    ...
    if (circularStatus != 0 && flag == 0) {
        $.ajax({
            ...
            success: function (Result) {
                ...
                $("[id*=hdcdrw]").val(dtls[1]);
                $("[id*=hddevwork]").val(dtls[0]);
                //alert($("[id*=hddevwork]").val());
                totman = parseInt(totman)+parseInt(dtls[0]) + parseInt(dtls[1]);
                totcst = parseFloat(parseInt(totman) * parseInt(cst)).toFixed(2);
                itmdata = '';
                Data = $("[id*=hddraftid]").val() + 'µ' + reqid + 'µ' + $("[id*=hdUserId]").val() + 'µ' + dtls[0] + 'µ' + '0' + 'µ' + dtls[1] + 'µ' + totman + 'µ' + totcst + 'µ' + rmk + 'µ' + circularStatus + 'µ' + module + 'µ';
                //alert(Data);
                $("[id*=hdDetails]").val(Data);
